...
const MessageComponent = memo(() => {
  ...
  return (
    <div className="container" style={{ display: 'flex', flexDirection: 'row' }}>
      <div
        className=""
        style={{ display: 'flex', flexDirection: 'column', maxWidth: '30%', width: '25%' }}
      >
        {contacts.map((c) => {
          return (
            <div key={c} className="card" onClick={openChat}>
      ...
      >
        <div className="container" style={{ height: '80vh', overflowY: 'scroll', padding: '1rem' }}>
          {messages.map((m, index) => {
            return (
              <div
                key={index}
                className="card col-6 p-1 bg-primary"
                style={{ marginLeft: m.sender === 'Me' ? 'none' : 'auto' }}
              >
        ...
        <form className="form form-control message-input-form" style={{ marginTop: 'auto' }}>
          <textarea
            className="input col-12 message-input"
            rows="4"
            cols="50"
